...
import { LayoutDashboard, User, BarChart3, Table, Menu, ChevronRight } from 'lucide-react';
import { useState } from 'react';
  ...
  return (
    <aside className={`${sidebarOpen ? 'w-64' : 'w-20'} bg-gray-800 text-white transition-all duration-300 flex flex-col h-screen`}>
      ...
      <nav className="flex-1 p-4">
        ...
        {/* PAGES Section */}
        <div className="mb-6">
          <p className={`text-xs text-gray-400 mb-2 uppercase tracking-wide ${!sidebarOpen && 'hidden'}`}>Pages</p>
          <Link 
            href="/dashboard/profile"
            className={`w-full flex items-center gap-3 p-2 rounded hover:bg-gray-700 mb-1 ${
