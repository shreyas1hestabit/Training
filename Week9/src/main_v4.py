...
#     print("Commands: /memory  /facts  /episodes  /clear  /reset  exit\n")
...
#             log_command("/reset", {"result": "session + vector store wiped"})
#             continue

#         # ── Normal chat turn ─────────────────────────────────────────

#         # Capture recalled memories BEFORE the chat call so we can log them
#         recalled = []
...
async def main():
    ...
    while True:
        ...
        if user_input.lower() == "exit":
            ...
            # Deletes all stored facts — useful after hallucination incidents
            facts = agent.recall_all_facts()
