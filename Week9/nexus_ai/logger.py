...
nexus_ai/logger.py
------------------
Structured trace logger for NEXUS AI.
Every agent call, tool use, critic review, and validation is logged
to logs/nexus_trace.json with full inputs, outputs, and timing.
"""
...
TRACE_FILE = LOGS_DIR / "nexus_trace.json"


class NexusLogger:
    """Append-only JSON trace logger."""
    ...
    def tool_call(self, tool: str, args: dict, result: str):
        ...
        print(f"  [TOOL] {tool}({list(args.keys())}) → {result[:60]}...")
    ...
    def validation_result(self, is_valid: bool, report: str):
        self._log("VALIDATION", "validator", {
            "is_valid": is_valid,
            "report":   report[:300],
        })
    ...
    def _flush(self):
        """Append this task's entries to the trace file."""
        existing = []
