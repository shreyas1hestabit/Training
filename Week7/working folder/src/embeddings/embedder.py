...
import torch #to check if we have gpu or not and if not then work on cpu
import numpy as np #use it at the end to format numbers properly.


class Embedder:
    def __init__(self, model_name="BAAI/bge-small-en-v1.5"):
        #used this baai/bge-small-en-v1.5 model beacuse it is tiny yet high performing. it is fast and does not require a massive computer but it ranks high on leaderboard for understanding english.
        ...
        #cuda is the branch name for nvidia's gpu technology
    ...
    def embed(self, texts, batch_size=32): # texts is a list of string and batch tells the ai to process 32 sentences at a time. saves time.
        ...
        return np.array(embeddings, dtype="float32") #converts the output to a numpy array and ensures that numbers are stored in 32 bit decimals (standard format) to save RAM
