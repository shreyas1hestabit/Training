...
def load_pdf(file_path):  #ek function bnaya load_pdf naam se which will load the pdf stored in file_path.
    reader=PdfReader(file_path)  # creates an object which tells the library to go to the file path and prepare pdf reading
    #pdf reader will not only open the pdf but will also parse through the pdf's internal structure(the cross-refernce table) so it knows exactly where each page starts and ends in the byte data.
    ...
    for i,page in enumerate(reader.pages): #render.pages is the list of all the pages inside the pdf
        ...
        # so agr hm iski work flow smjhe toh its like ki reader.pages give the content of the page and acts like the WHAT whereas enumerate acts like the WHERE which is just a number like 0,1,2... this gives us the page number 
        ...
        if text: #this checks if the variable is truthy i.e. if a page is blank or contains only image then extract_text might return an empty string. this line ensures that we dont save empty data.
            ...
            #tuple because then it is stored as immutable pair
